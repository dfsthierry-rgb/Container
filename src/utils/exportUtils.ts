...
export function exportToPDF(lancamentos: Lancamento[]) {
    ...
    const head = [[
        'Tipo', 'Ref', 'DI', 'Invoices', 'Dt.Emb', 'Exportador', 'Dt.Cheg', 'Proc.', 'Navio', 'Tp.Carg', 'Armador', 'L.Emb', 'L.Nac', 'Incoterm', 'Dt.Reg', 'Câmbio', 'FOB($)', 'Frete', 'Seguro', 'Acrés', 'Capat', 'II', 'IPI', 'PIS', 'COFINS', 'ICMS', 'Multa', 'AFRMM', 'Sisc', 'Out.DA', 'Armaz', 'Desc.', 'Tr.Int', 'Lib.', 'Hon.', 'Prest.', 'Desp.Div', 'FOB(R$)', 'CIF', 'V.Adu', 'Log.', 'Trib.', 'Desp.Ad', 'Total', 'Fator'
    ]];

    const tableData = lancamentos.map(l => {
        const calc = computeLanc(l);
        return [
            l.tipo_container || '-', l.ref_montreal || '-', l.di || '-', l.invoices || '-', l.data_embarque || '-', l.exportadores || '-', l.data_chegada || '-', l.procedencia || '-', l.navio || '-', l.tipo_carga || '-', l.armador || '-', l.local_embarque || '-', l.local_nacionalizacao || '-', l.incoterm || '-', l.data_registro || '-',
